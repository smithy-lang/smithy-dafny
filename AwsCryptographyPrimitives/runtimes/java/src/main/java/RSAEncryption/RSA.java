...
public class RSA {
  ...
  private static String RSA_ERROR_MSG = String.format(
    "AWS Crypto will not generate an RSA Key with length greater than %s",
    RSA_KEY_LEN_MAX
  );
  ...
  public static Tuple2<
    ...
    DafnySequence<? extends Byte>
  > GenerateKeyPairExtern(int lengthBits) {
    try {
  ...
  public static Result<Integer, Error> GetRSAKeyModulusLengthExtern(
  ...
  // GetPemBytes represents a helper method that takes an AsymmetricKeyParameter and returns the
  // corresponding
  ...
  private static DafnySequence<? extends Byte> GetPemBytes(
    AsymmetricKeyParameter keyParameter
  ) throws IOException {
    if (keyParameter.isPrivate()) {
      PrivateKeyInfo privateKey = PrivateKeyInfoFactory.createPrivateKeyInfo(
        keyParameter
      );
  ...
  private static RSAKeyParameters ParsePublicRsaPemBytes(byte[] pem)
    throws IOException {
    ...
    RSAKeyParameters publicKeyParams =
      (RSAKeyParameters) PublicKeyFactory.createKey(content);
    return publicKeyParams;
  }

  // Parses UTF8-encoded, PEM-encoded RSA Private keys.
  private static RSAKeyParameters ParsePrivateRsaPemBytes(byte[] pem)
    throws IOException {
    PemReader pemReader = new PemReader(
      new InputStreamReader(new ByteArrayInputStream(pem))
    );
    PemObject pemObject = pemReader.readPemObject();
    byte[] content = pemObject.getContent();
    RSAKeyParameters privateKeyParams =
      (RSAKeyParameters) PrivateKeyFactory.createKey(content);
  ...
  private static AsymmetricBlockCipher GetEngineForPadding(
    ...
    if (paddingMode.is_OAEP__SHA1()) {
      ...
      throw new RuntimeException(
        String.format("Invalid RSA Padding Scheme %s", paddingMode)
      );
    }
  }
