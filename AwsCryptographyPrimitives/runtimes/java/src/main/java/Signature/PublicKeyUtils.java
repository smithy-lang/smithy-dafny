...
class PublicKeyUtils {
   ...
   */
  static byte[] encodeAndCompressPublicKey(
    PublicKey key,
  ...
  static Result<ECPublicKey, Error> decodePublicKey(
    ...
    try {
      final ECParameterSpec ecParameterSpec =
        SignatureAlgorithm.ecParameterSpec(algorithm);
      final byte[] keyAsBytes = (byte[]) Array.unwrap(
        dtor_verificationKey.toArray()
      );
      final ECPublicKeySpec publicKeySpec = new ECPublicKeySpec(
        byteArrayToECPoint(keyAsBytes, ecParameterSpec),
        ecParameterSpec
      );
   ...
   *
   * <p>Original Author: Wesley Rosenblum
   *
  ...
  static ECPoint byteArrayToECPoint(
    ...
    final BigInteger x = new BigInteger(
      1,
      Arrays.copyOfRange(keyAsBytes, 1, keyAsBytes.length)
    );
    ...
    final BigInteger p =
      ((ECFieldFp) ecParameterSpec.getCurve().getField()).getP();
