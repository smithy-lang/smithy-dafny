...
public class ModeledShapeValueTest extends ForEachDafnyTest {
    ...
    static {
        EXPECTED_VALUES.put("NoParameters",
                """
                software.amazon.polymorph.model.MyNestedStructure.builder()
                .build()
                """);
        EXPECTED_VALUES.put("StressTest",
            """
            software.amazon.polymorph.model.MyNestedStructure.builder()
    ...
    public ModeledShapeValueTest(DafnyVersion dafnyVersion) {
        ...
        underTest = TestSetupUtils.setupLibrary(
                model,
                "aws.polymorph",
                dafnyVersion
        );
    ...
    public void shapeValueTests() {
        ArrayNode testCases = (ArrayNode)underTest.model.getMetadata().get("testCases");
    ...
    public void assertTestCaseCorrect(Node testCase) {
        ...
        ShapeId shapeId = ShapeId.from(testCaseNode.expectStringMember("shape").getValue());
        ...
        CodeBlock actual = ModeledShapeValue.shapeValue(underTest, false, shape, value);
        String expected = EXPECTED_VALUES.get(testId);
        tokenizeAndAssertEqual(expected, actual.toString());
    }

    private void validateTestCase(Model model, String id, Shape shape, Node node) {
