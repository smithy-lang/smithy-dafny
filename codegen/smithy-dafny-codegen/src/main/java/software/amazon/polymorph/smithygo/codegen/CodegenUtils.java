...
public final class CodegenUtils {
  ...
  public static String getSymbolNamespaceAlias(Symbol symbol) {
    return symbol
      .getProperty(SymbolUtils.NAMESPACE_ALIAS, String.class)
  ...
  public static String asAddressIfAddressable(
    ...
    if (shape.isMemberShape()) {
      isStruct =
        model.expectShape(shape.asMemberShape().get().getTarget()).getType() ==
        ShapeType.STRUCTURE;
    }
  ...
  public static MapShape expectMapShape(Shape shape) {
    ...
    throw new CodegenException(
      "expect shape " + shape.getId() + " to be Map, was " + shape.getType()
    );
  }
   ...
   * @return MemberShape if found
   */
  public static MemberShape expectMember(
    StructureShape shape,
    Predicate<String> memberNamePredicate
  ) {
    return shape
      .getAllMembers()
      .values()
      ...
      .orElseThrow(() ->
        new CodegenException(
          "did not find member in structure shape, " + shape.getId()
        )
      );
  }
