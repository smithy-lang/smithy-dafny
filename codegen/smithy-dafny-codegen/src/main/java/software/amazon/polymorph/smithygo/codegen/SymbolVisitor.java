...
public class SymbolVisitor implements SymbolProvider, ShapeVisitor<Symbol> {
  ...
  > structureSpecificMemberEscapers = new HashMap<>();
  ...
  public SymbolVisitor(Model model, GoSettings settings) {
    ...
    this.rootModuleName =
      SmithyNameResolver.shapeNamespace(settings.getService(model));
    ...
    escaper =
      ReservedWordSymbolProvider
        ...
        .escapePredicate((shape, symbol) ->
          !StringUtils.isEmpty(symbol.getDefinitionFile())
        )
        .buildEscaper();
    ...
    errorMemberEscaper =
      ReservedWordSymbolProvider
        .builder()
        .memberReservedWords(
          ReservedWords.compose(reservedMembers, reservedErrorMembers)
        )
        .escapePredicate((shape, symbol) ->
          !StringUtils.isEmpty(symbol.getDefinitionFile())
        )
   ...
   */
  private void reserveUnionMemberNames(
    Model model,
    ReservedWordsBuilder builder
  ) {
    model
      .shapes(UnionShape.class)
      .forEach(union -> {
  ...
  private void reserveInterfaceMemberAccessors(StructureShape shape) {
    ...
    ReservedWordSymbolProvider.Escaper structureSpecificMemberEscaper =
      ReservedWordSymbolProvider
        .builder()
        .memberReservedWords(builder.build())
        .buildEscaper();
    structureSpecificMemberEscapers.put(
      shape.getId(),
      structureSpecificMemberEscaper
    );
  }
  ...
  public Symbol toSymbol(Shape shape) {
    ...
    LOGGER.fine(() ->
      String.format("Creating symbol from %s: %s", shape, symbol)
    );
  ...
  public String toMemberName(MemberShape shape) {
    ...
    if (structureSpecificMemberEscapers.containsKey(shape.getContainer())) {
      memberName =
        structureSpecificMemberEscapers
          .get(shape.getContainer())
          .escapeMemberName(memberName);
    }
  ...
  private String getDefaultShapeName(Shape shape) {
    ServiceShape serviceShape = model.expectShape(
      settings.getService(),
      ServiceShape.class
    );
    if (shape.hasTrait(ServiceTrait.class)) {
      return DafnyNameResolver.getDafnyClientName(shape.expectTrait(ServiceTrait.class).getSdkId());
    }
  ...
  private String getDefaultMemberName(MemberShape shape) {
    String memberName = StringUtils.capitalize(
  ...
  private String removeLeadingInvalidIdentCharacters(String value) {
    ...
    if (remaining.length() == 0) {
      throw new CodegenException(
        "tried to clean name " + value + ", but resulted in empty string"
      );
    }
  ...
  public Symbol blobShape(BlobShape shape) {
    if (shape.hasTrait(StreamingTrait.ID)) {
      Symbol inputVariant = symbolBuilderFor(
        shape,
        "Reader",
        SmithyGoDependency.IO
      )
        .build();
  ...
  private Symbol createCollectionSymbol(CollectionShape shape) {
    ...
    return symbolBuilderFor(shape, getDefaultShapeName(shape))
      ...
      .putProperty(
        SymbolUtils.GO_UNIVERSE_TYPE,
        reference
          .getProperty(SymbolUtils.GO_UNIVERSE_TYPE, Boolean.class)
          .orElse(false)
      )
  ...
  private Symbol createBigSymbol(Shape shape, String symbolName) {
    return symbolBuilderFor(shape, symbolName, SmithyGoDependency.BIG).build();
  }
  ...
  public Symbol operationShape(OperationShape shape) {
    ...
    return SymbolUtils
      .createPointableSymbolBuilder(shape, name, rootModuleName)
  ...
  public Symbol serviceShape(ServiceShape shape) {
    return symbolBuilderFor(shape, "Client", rootModuleName)
      .definitionFile(
        "./%s/api_client.go".formatted(SmithyNameResolver.shapeNamespace(shape))
      )
      .build();
  }
  ...
  public Symbol structureShape(StructureShape shape) {
    ...
    if (
      shape
        .getId()
        .getNamespace()
        .equals(CodegenUtils.getSyntheticTypeNamespace())
    ) {
  ...
  private Optional<String> getNameOfBoundOperation(StructureShape shape) {
    ...
    for (Relationship relationship : provider.getNeighbors(shape)) {
      ...
      if (
        ...
        return Optional.of(
          getDefaultShapeName(relationship.getNeighborShape().get())
        );
      }
    }
