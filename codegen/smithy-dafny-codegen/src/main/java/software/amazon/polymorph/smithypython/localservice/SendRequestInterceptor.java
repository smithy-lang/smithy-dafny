...
/**
 * Dafny plugin's interceptor for SendRequestSection. By default, Smithy-Python will generate code
 * to make an HTTP request in this section. Dafny plugin overrides this to interact with
 * DafnyImplInterface. Since the Appender interface doesn't implement a mechanism to pass in
 * GenerationContext, we cannot use the Smithy model to decide how to write this section, and this
 * section must be written as static (unchanging) code. To work around this, this section interfaces
 * with the static DafnyImplInterface, which contains internal logic that *can* access the Smithy
 * model.
 */
final class SendRequestInterceptor implements Appender<SendRequestSection, PythonWriter> {
  @Override
