...
public class DafnyAwsSdkClientTypeConversionProtocol implements ProtocolGenerator {
    final Model dafnyNonNormalizedModel;
    final Model awsNormalizedModel;
    final ServiceShape serviceShape;
    public DafnyAwsSdkClientTypeConversionProtocol(Model model, ServiceShape serviceShape) {
        dafnyNonNormalizedModel = model;
        awsNormalizedModel = AddOperationShapes.execute(model, serviceShape.toShapeId());
    ...
    public void generateSerializers(GenerationContext context) {
        ...
        serviceShape.getOperations().forEach(eachOperation -> {
            final var awsNormalizedOperation = awsNormalizedModel.expectShape(eachOperation, OperationShape.class);
            ...
            if (!alreadyVisited.contains(awsNormalizedInputShape.toShapeId())) {
                ...
                if (!awsNormalizedInputShape.hasTrait(UnitTypeTrait.class) && awsNormalizedInputShape.toShapeId().getNamespace().equals(serviceShape.toShapeId().getNamespace())) {
                    final var awsNormalizedInputToDafnyMethodName = SmithyNameResolver.getToDafnyMethodName(serviceShape, awsNormalizedInputShape, "");
                    final var awsNormalizedInputSymbol = symbolProvider.toSymbol(awsNormalizedInputShape);
                    final var dafnyInput = dafnyNonNormalizedModel.expectShape(eachOperation, OperationShape.class).getInputShape();
                    final var dafnyInputSymbol = symbolProvider.toSymbol(dafnyNonNormalizedModel.expectShape(dafnyInput));
                    writerDelegator.useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(serviceShape), TO_DAFNY), SmithyNameResolver.shapeNamespace(serviceShape), writer -> {
                        writer.addImport(SmithyNameResolver.getGoModuleNameForSdkNamespace(awsNormalizedInputShape.toShapeId().getNamespace()));
                        writer.write("""
            ...
            if (!alreadyVisited.contains(awsNormalizedOutputShape.toShapeId())) {
                ...
                if (!awsNormalizedOutputShape.hasTrait(UnitTypeTrait.class) && awsNormalizedOutputShape.toShapeId().getNamespace().equals(serviceShape.toShapeId().getNamespace())) {
                    final var awsNormalizedOutputToDafnyMethodName = SmithyNameResolver.getToDafnyMethodName(serviceShape, awsNormalizedOutputShape, "");
                    final var awsNormalizedOutputSymbol = symbolProvider.toSymbol(awsNormalizedOutputShape);
                    final var dafnyOutput = dafnyNonNormalizedModel.expectShape(eachOperation, OperationShape.class).getOutputShape();
                    final var dafnyOutputSymbol = symbolProvider.toSymbol(dafnyNonNormalizedModel.expectShape(dafnyOutput));
                    writerDelegator.useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(serviceShape), TO_DAFNY), SmithyNameResolver.shapeNamespace(serviceShape), writer -> {
                        writer.addImport(SmithyNameResolver.getGoModuleNameForSdkNamespace(awsNormalizedInputShape.toShapeId().getNamespace()));
                        writer.write("""
    ...
    public void generateDeserializers(GenerationContext context) {
        ...
        serviceShape.getOperations().forEach(eachOperation -> {
            final var awsNormalizedOperationShape = awsNormalizedModel.expectShape(eachOperation, OperationShape.class);
            final var awsNormalizedInputShape = awsNormalizedModel.expectShape(awsNormalizedOperationShape.getInputShape());
            if (!alreadyVisited.contains(awsNormalizedInputShape.toShapeId())) {
                alreadyVisited.add(awsNormalizedInputShape.toShapeId());
                if (!awsNormalizedInputShape.hasTrait(UnitTypeTrait.class) && awsNormalizedInputShape.toShapeId().getNamespace().equals(serviceShape.toShapeId().getNamespace())) {
                    final var awsNormalizedInputFromDafnyMethodName = SmithyNameResolver.getFromDafnyMethodName(serviceShape, awsNormalizedInputShape, "");
                    final var awsNormalizedInputSymbol = symbolProvider.toSymbol(awsNormalizedInputShape);
                    final var dafnyInput = dafnyNonNormalizedModel.expectShape(eachOperation, OperationShape.class).getInputShape();
                    final var dafnyInputSymbol = symbolProvider.toSymbol(dafnyNonNormalizedModel.expectShape(dafnyInput));
            ...
            final var awsNormalizedOutputShape = awsNormalizedModel.expectShape(awsNormalizedOperationShape.getOutputShape());
            if (!alreadyVisited.contains(awsNormalizedOutputShape.toShapeId())) {
                alreadyVisited.add(awsNormalizedOutputShape.toShapeId());
                if (!awsNormalizedOutputShape.hasTrait(UnitTypeTrait.class) && awsNormalizedOutputShape.toShapeId().getNamespace().equals(serviceShape.toShapeId().getNamespace())) {
                    final var awsNormalizedOutputFromDafnyMethodName = SmithyNameResolver.getFromDafnyMethodName(serviceShape, awsNormalizedOutputShape, "");
                    final var awsNormalizedOutputSymbol = context.symbolProvider().toSymbol(awsNormalizedOutputShape);
                    final var dafnyOutput = dafnyNonNormalizedModel.expectShape(eachOperation, OperationShape.class).getOutputShape();
                    final var dafnyOutputSymbol = symbolProvider.toSymbol(dafnyNonNormalizedModel.expectShape(dafnyOutput));
                    delegator.useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(serviceShape), TO_NATIVE), SmithyNameResolver.shapeNamespace(serviceShape), writer -> {
                        writer.addImport(SmithyNameResolver.getGoModuleNameForSdkNamespace(awsNormalizedInputShape.toShapeId().getNamespace()));
    ...
    private void generateRequestSerializer(
        ...
        final var nonNormalizedOperation = dafnyNonNormalizedModel.expectShape(operation.toShapeId(), OperationShape.class);
        final var targetShape = dafnyNonNormalizedModel.expectShape(nonNormalizedOperation.getInputShape());
        delegator.useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(operation), TO_DAFNY), SmithyNameResolver.shapeNamespace(operation), writer -> {
                                    final var input = targetShape.accept(new AwsSdkToDafnyShapeVisitor(
                                            context,
                                            "nativeInput",
                                            writer,
                                            false, false, false
    ...
    private void generateResponseSerializer(
        ...
        final var nonNormalizedOperation = dafnyNonNormalizedModel.expectShape(operation.toShapeId(), OperationShape.class);
        final var targetShape = dafnyNonNormalizedModel.expectShape(nonNormalizedOperation.getOutputShape());
        delegator.useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(operation), TO_DAFNY), SmithyNameResolver.shapeNamespace(operation), writer -> {
                                    final var input = targetShape.accept(new AwsSdkToDafnyShapeVisitor(
                                            context,
                                            "nativeOutput",
    ...
    private void generateRequestDeserializer(
        ...
        delegator.useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(operation), TO_NATIVE), SmithyNameResolver.shapeNamespace(operation), writer -> {
            ...
            final var input = targetShape.accept(new DafnyToAwsSdkShapeVisitor(
                    context,
                    "dafnyInput",
                    writer
            ));
    ...
    private void generateResponseDeserializer(
        ...
        delegator.useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(operation), TO_NATIVE), SmithyNameResolver.shapeNamespace(operation), writer -> {
            ...
            final var output = targetShape.accept(new DafnyToAwsSdkShapeVisitor(
                    context,
                    "dafnyOutput",
                    writer
            ));
    ...
    private void generateErrorSerializer(final GenerationContext context) {
        final Set<ShapeId> alreadyVisited = new HashSet<>();
        final var errorShapes = awsNormalizedModel.getShapesWithTrait(ErrorTrait.class);
            ...
            if (!alreadyVisited.contains(errorShape.toShapeId())) {
                ...
                context.writerDelegator().useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(errorShape), TO_DAFNY), SmithyNameResolver.shapeNamespace(errorShape), writer -> {
                    ...
                    writer.write("""
                                 ...
                                 getInputToDafnyMethodName, context.symbolProvider().toSymbol(errorShape).getName(), DafnyNameResolver.getDafnyBaseErrorType(errorShape),
                                 writer.consumer(w -> {
                                     String output = errorShape.accept(new AwsSdkToDafnyShapeVisitor(
                                             context,
                                             "nativeInput",
                                             writer,
                                             false, false, false
        ...
        context.writerDelegator()
               .useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(serviceShape), TO_DAFNY),
                              SmithyNameResolver.shapeNamespace(serviceShape), writer -> {
                           writer.write("""
    ...
    private void generateErrorDeserializer(final GenerationContext context) {
            ...
            if (!alreadyVisited.contains(errorShape.toShapeId())) {
                ...
                context.writerDelegator().useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(errorShape), TO_NATIVE), SmithyNameResolver.shapeNamespace(errorShape), writer -> {
                    ...
                    writer.write("""
                                 ...
                                 getOutputFromDafnyMethodName, DafnyNameResolver.getDafnyBaseErrorType(errorShape), context.symbolProvider().toSymbol(errorShape).getName(),
                                 writer.consumer(w -> {
                                     String output = errorShape.accept(new DafnyToAwsSdkShapeVisitor(
                                             context,
                                             "dafnyOutput",
                                             writer
                                     ));
        ...
        context.writerDelegator()
               .useFileWriter("%s/%s".formatted(SmithyNameResolver.shapeNamespace(serviceShape), TO_NATIVE),
                              SmithyNameResolver.shapeNamespace(serviceShape), writer -> {
                           writer.write("""
