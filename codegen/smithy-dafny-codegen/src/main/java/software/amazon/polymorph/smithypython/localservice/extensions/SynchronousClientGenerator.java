...
public class SynchronousClientGenerator extends ClientGenerator {
    ...
    writer.openBlock(
        ...
        () -> {
          ...
          for (ShapeId operationShapeId : service.getOperations()) {
            generateOperation(
                writer, context.model().expectShape(operationShapeId).asOperationShape().get());
          }
        });
              ...
              () -> {
                ...
                var docs =
                    operation
                        ...
                        .orElse(
                            String.format(
                                "Invokes the %s operation.", operation.getId().getName()));
  ...
  //                            # with different names and type hints. It is possible to address
  // this
  //                            # without having to fall back to the type ignore, but it would
  // impose
  ...
  //                            # config.logger.exception(f"Exception occurred while handling:
  // {context.response}")
  //                            pass
  //                        context._response = e
  //
  //                    # At this point, the context's request will have been definitively set, and
  //                    # The response will be set either with the modeled output or an exception.
  // The
  ...
  //      writer.addImports("smithy_python.interfaces.auth", Set.of("HTTPSigner",
  // "HTTPAuthOption"));
  ...
  //      writer.addDependency(SmithyPythonDependency.SMITHY_PYTHON);
  //      // TODO: implement the endpoints 2.0 spec and remove the hard-coded handling of static
  // params.
  ...
  //                        request_config = config.http_request_config or
  // HTTPRequestConfiguration()
  //                        context_with_response = cast(
  //                            InterceptorContext[Input, None, $1T, $2T], context
  //                        )
  //                        context_with_response._transport_response = await
  ...
  //                        context._response = e
  //
  //                    # At this point, the context's request and transport_request have
  // definitively been set,
  //                    # the response is either set or an exception, and the transport_resposne is
  // either set or
  //                    # None. This will also be true after _finalize_attempt because there is no
  // opportunity
  ...
  //                            # config.logger.exception(f"Exception occurred while dispatching
  // trace events: {e}")
  //                            pass
  //                    except Exception as e:
  //                        if context.response is not None:
  ...
  //
  //                    # We may want to add some aspects of this context to the output types so we
  // can
