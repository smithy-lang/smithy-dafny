...
public final class DafnyPythonLocalServiceIntegration implements PythonIntegration {
      ...
      RuntimeClientPlugin.builder()
          ...
          .pythonPlugin(
              SymbolReference.builder()
                  .symbol(
                      Symbol.builder().name("set_config_impl").namespace(".plugin", ".").build())
                  .build())
          .build();
  ...
  public void customize(GenerationContext codegenContext) {
    ...
    if (!codegenContext
        .applicationProtocol()
        .equals(
            DafnyPythonLocalServiceProtocolGenerator
                .DAFNY_PYTHON_LOCAL_SERVICE_APPLICATION_PROTOCOL)) {
      return;
    }

    // Generate customizations for service shapes with localService trait
    Set<ServiceShape> serviceShapes =
        Set.of(
            codegenContext
                .model()
    ...
    Set<ShapeId> serviceOperationShapes =
        serviceShapes.stream()
            .map(EntityShape::getOperations)
            .flatMap(Collection::stream)
            .collect(Collectors.toSet());
    Set<ShapeId> nonServiceOperationShapes =
        codegenContext.model().getOperationShapes().stream()
            .map(Shape::getId)
            .filter(
                operationShapeId ->
                    operationShapeId.getNamespace().equals(serviceShape.getId().getNamespace()))
            .collect(Collectors.toSet());
    nonServiceOperationShapes.removeAll(serviceOperationShapes);

    Set<Shape> referenceShapes =
        codegenContext.model().getStructureShapesWithTrait(ReferenceTrait.class).stream()
