...
import software.amazon.smithy.dafny.codegen.runtime.java.generator.library.JavaLibrary.MethodSignature;
...
public abstract class ShimLibrary extends Generator {
    ...
    protected JavaLibrary.MethodSignature operationMethodSignature(OperationShape shape) {
        ...
        if (!inputResolved.resolvedId().equals(Constants.SMITHY_API_UNIT)) {
            TypeName inputType = methodSignatureTypeName(inputResolved);
            method.addParameter(inputType, NATIVE_VAR);
        }
        // if operation is not void
        if (!outputResolved.resolvedId().equals(Constants.SMITHY_API_UNIT)) {
    ...
    protected MethodSpec operation(OperationShape operationShape) {
        ...
        TypeName success = Constants.DAFNY_TUPLE0_CLASS_NAME;
        // if operation is not void
        if (!outputResolved.resolvedId().equals(Constants.SMITHY_API_UNIT)) {
