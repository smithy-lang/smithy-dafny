...
import software.amazon.smithy.dafny.codegen.runtime.dotnet.TypeConversionDirection;
    ...
    public TokenTree generateOperationShim(final ShapeId operationShapeId) {
        ...
        final TokenTree unWrappedRequest = Token.of(operationShape.getInput()
                .map(requestShapeId -> "%s unWrappedRequest = %s(request);".formatted(
                        ...
                        nameResolver.qualifiedTypeConverter(requestShapeId, TypeConversionDirection.FROM_DAFNY)))
                .orElse(""));
        ...
        final TokenTree returnResponse = Token.of(operationShape.getOutput()
                .map(responseShapeId -> "return %s.create_Success(%s(wrappedResponse));".formatted(
                        dafnyOutputType,
                        nameResolver.qualifiedTypeConverter(responseShapeId, TypeConversionDirection.TO_DAFNY)))
    ...
    public TokenTree generateErrorTypeShim() {
        ...
        final TokenTree knownErrorCases = TokenTree.of(errorShapes.stream()
                .map(errorShape -> {
                    ...
                    final String errorConverter = DotNetNameResolver.qualifiedTypeConverter(errorShapeId, TypeConversionDirection.TO_DAFNY);
                    return Token.of("""
                            case %s e:
                                return %s(e);
