...
module SimpleBlobImpl refines AbstractSimpleTypesBlobOperations  {
    ...
    returns (output: Result<GetBlobOutput, Error>)
  {
    ...
    //   implementation. In the example above, if the copy-by-value is incorrect, we would not know by testing it from
    //   outside of this function.
    // 
    // However, in order to validate the value of `input` from within this function, we must test against a known Dafny
    //   value. This value also cannot be passed into the function. As in the example above, this known value may also
    //   be copied by value, and potentially suffer the same mutations described above.
