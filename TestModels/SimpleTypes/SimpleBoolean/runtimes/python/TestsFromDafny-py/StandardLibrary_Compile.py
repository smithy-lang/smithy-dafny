...
assert "StandardLibrary_Compile" == __name__
StandardLibrary_Compile = sys.modules[__name__]
...
class default__:
    ...
    def __dafnystr__(self) -> str:
        return "StandardLibrary_Compile._default"
    @staticmethod
    def Join(ss, joiner):
    ...
    def Split(s, delim):
        ...
        while True:
            with _dafny.label():
                d_52_i_ = StandardLibrary_Compile.default__.FindIndexMatching(s, delim, 0)
                if (d_52_i_).is_Some:
    ...
    def SplitOnce(s, delim):
        d_53_i_ = StandardLibrary_Compile.default__.FindIndexMatching(s, delim, 0)
        return (_dafny.Seq((s)[:(d_53_i_).value:]), _dafny.Seq((s)[((d_53_i_).value) + (1)::]))

    @staticmethod
    def SplitOnce_q(s, delim):
        d_54_valueOrError0_ = StandardLibrary_Compile.default__.FindIndexMatching(s, delim, 0)
        if (d_54_valueOrError0_).IsFailure():
            return (d_54_valueOrError0_).PropagateFailure()
        elif True:
            d_55_i_ = (d_54_valueOrError0_).Extract()
            return Wrappers_Compile.Option_Some((_dafny.Seq((s)[:d_55_i_:]), _dafny.Seq((s)[(d_55_i_) + (1)::])))
    ...
    def FindIndexMatching(s, c, i):
        ...
        return StandardLibrary_Compile.default__.FindIndex(s, lambda0_(c), i)
        ...
        while True:
            with _dafny.label():
                if (i) == (len(s)):
                    ...
                    return Wrappers_Compile.Option_Some(i)
                elif True:
                    in4_ = s
                    in5_ = f
                    in6_ = (i) + (1)
    ...
    def SeqToArray(s):
        ...
        arrayinit0_ = lambda2_(s)
        for arrayinit_00_ in range(nw0_.length(0)):
            nw0_[arrayinit_00_] = arrayinit0_(arrayinit_00_)
        a = nw0_
        return a
    ...
    def LexicographicLessOrEqual(a, b, less):
        def lambda4_(exists_var_0_):
            ...
            return (((0) <= (d_62_k_)) and ((d_62_k_) <= (len(a)))) and (StandardLibrary_Compile.default__.LexicographicLessOrEqualAux(a, b, less, d_62_k_))
                    ...
                    def iife0_(_let_dummy_0):
                        ...
                        with _dafny.label("_ASSIGN_SUCH_THAT_d_0"):
                            ...
                            for assign_such_that_0_ in (s).Elements:
                                ...
                                if ((d_65_a_) in (s)) and (StandardLibrary_Compile.default__.IsMinimum(d_65_a_, s, less)):
                                    raise _dafny.Break("_ASSIGN_SUCH_THAT_d_0")
                            raise Exception("assign-such-that search produced no value (line 369)")
                            pass
                        return (_dafny.Seq([d_65_a_])) + (StandardLibrary_Compile.default__.SetToOrderedSequence((pat_let_tv0_) - (_dafny.Set({d_65_a_})), pat_let_tv1_))
                    return iife0_(0)
    ...
    def IsMinimum(a, s, less):
        def lambda6_(forall_var_1_):
            ...
            return not ((d_66_z_) in (s)) or (StandardLibrary_Compile.default__.LexicographicLessOrEqual(a, d_66_z_, less))
