...
module SimpleTimestampImplTest {
  ...
  method {:test} TestClient(){
    ...
    TestGetTimestampRequired(client);
  ...
  method TestGetTimestampRequired(client: ISimpleTypesTimestampClient)
    requires client.ValidState()
    modifies client.Modifies
    ensures client.ValidState()
  {
    var dafnyTimestamp := "2024-06-11T12:34:56.789Z";
    var ret :- expect client.GetTimestampRequired(SimpleTimestamp.Types.GetTimestampRequiredInput(value := dafnyTimestamp));
    expect ret.value == dafnyTimestamp;
    print ret;
  }
