...
# In any folder like SimpleTypes/SimpleString, after running
#     DAFNY_VERSION=4.3.0 make polymorph_dafny
# Run
#     DAFNY=<path to feat-rust of Dafny executable> make dafny_benerate
# This will compile the Dafny file to Rust and place it in the appropriate position
...
dafny_benerate: DAFNY_RUST_OUTPUT_RS_PROCESSED=\
	$(DAFNY_RUST_OUTPUT)-rust/implementation_from_dafny.rs
dafny_benerate: DAFNY_RUST_IMPL=\
	runtimes/rust/dafny_impl/src/implementation_from_dafny.rs
dafny_benerate:
	$(DAFNY) translate rs src/Index.dfy --emit-uncompilable-code --no-verify \
	...
	--function-syntax:3 \
	--allow-warnings \
	--output $(DAFNY_RUST_OUTPUT) --library:../../dafny-dependencies/StandardLibrary/src/Index.dfy
	python -c "import sys; data = sys.stdin.buffer.read(); sys.stdout.buffer.write(data.replace(b'\npub mod', b'\npub use dafny_standard_library::implementation_from_dafny::*;\n\npub mod', 1) if b'\npub mod' in data else data)" \
	  < $(DAFNY_RUST_OUTPUT_RS) > $(DAFNY_RUST_OUTPUT_RS_PROCESSED);
	rustfmt $(DAFNY_RUST_OUTPUT_RS_PROCESSED);
	rm $(DAFNY_RUST_OUTPUT_RS);
	rm $(DAFNY_RUST_OUTPUT_RS_DTR);
	rm -r $(DAFNY_RUST_OUTPUT_FOLDER);
	echo "Diffs remaining till code generation works:"
	diff -u $(DAFNY_RUST_OUTPUT_RS_PROCESSED) $(DAFNY_RUST_IMPL) | wc -l
	echo "Writing to  $(DAFNY_RUST_IMPL)"
	mv $(DAFNY_RUST_OUTPUT_RS_PROCESSED) $(DAFNY_RUST_IMPL)