...
type Client struct {
	DafnyClient *SimpleAggregate.SimpleAggregateClient
}
...
func NewClient(clientConfig SimpleAggregateTypes.SimpleAggregateConfig) (*Client, error) {
	...
	var dafny_response = SimpleAggregate.Companion_Default___.SimpleAggregate(dafnyConfig)
	if dafny_response.Is_Failure() {
		panic("Client construction failed. This should never happen")
	}
	var dafnyClient = dafny_response.Extract().(*SimpleAggregate.SimpleAggregateClient)
	client := &Client{dafnyClient}
	return client, nil
}

func (client *Client) GetAggregate(ctx context.Context, params SimpleAggregateTypes.GetAggregateInput) (*SimpleAggregateTypes.GetAggregateOutput, error) {
	err := params.Validate()
	if err != nil {
	...
	var dafny_request SimpleAggregate.GetAggregateInput_smithygenerated = GetAggregateInput_ToDafny(params)
	var dafny_response = client.DafnyClient.GetAggregate(dafny_request)

	if dafny_response.Is_Failure() {
		err := dafny_response.Dtor_error().(SimpleAggregate.Error)
...
func (client *Client) GetAggregateKnownValueTest(ctx context.Context, params SimpleAggregateTypes.GetAggregateInput) (*SimpleAggregateTypes.GetAggregateOutput, error) {
	err := params.Validate()
	if err != nil {
		opaqueErr := SimpleAggregateTypes.OpaqueError_smithygenerated{
			ErrObject: err,
