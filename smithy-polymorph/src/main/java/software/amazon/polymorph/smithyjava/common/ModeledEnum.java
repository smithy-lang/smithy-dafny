...
public class ModeledEnum {
        ...
        for (EnumDefinition value : enumTrait.getValues()) {
            if (value.getName().isEmpty()) {
                throw new IllegalArgumentException(
                        ...
                        "ShapeId: %s does not for value: %s".formatted(shape.getId(), value.getValue())
                );
            }
    ...
    private static TypeSpec enumValueTypeSpec(EnumDefinition value) {
        TypeSpec.Builder enumType = TypeSpec.anonymousClassBuilder("$S", value.getValue());
    ...
    private static MethodSpec constructor() {
        return MethodSpec.constructorBuilder()
                ...
                .addStatement("this.$L = $L", ENUM_VALUE_FIELD.name, ENUM_VALUE_FIELD.name)
                .build();
    }
    ...
    private static MethodSpec toStringMethod() {
        return MethodSpec.methodBuilder(TO_STRING_METHOD_NAME)
                ...
                .addModifiers(PUBLIC)
                .addStatement("return $T.valueOf($L)", String.class, ENUM_VALUE_FIELD.name)
                .build();
    }
}
